...
if ( ! function_exists( 'ref_enqueue_main_stylesheet' ) ) {
	function ref_enqueue_main_stylesheet() {
		...
		if ( is_shop() || is_page('create-a-new-youtube-collaboration') || is_page('edit-your-youtube-collaboration')   ) {
...
function sf_remove_scripts() {
    ...
    wp_deregister_style( 'wp-block-library' );
	    ...
	    wp_deregister_style( 'decomments' );

			wp_dequeue_style( 'decomments-ie' );
	    wp_deregister_style( 'decomments-ie' );
		}
...
//dequeu scripts and styles conditionally on campaign pages or blog posts
...
//dequeu scripts and styles conditionally on the homepage
...
//dequeue scripts and styles conditionally on campaign archive
...
function deregister_scripts5(){
	...
	global $post;
	if( is_post_type_archive( 'organizations' ) || is_page('checkout-choose-organization') ) :
		...
		$deregister_scripts = array('views-pagination-script1', 'woocommerce_views_onsale_badge_js', 'knockout', 'ddl-layouts-frontend',
						...
						'wptoolset-field-date', 'toolset-event-manager1', 'cred-frontend-js', 'toolset-select2-compatibility',
						'toolset_select2', 'cred-select2-frontend-js', 'ddl-tabs-scripts', 'zoom', 'flexslider', 'photoswipe', 'photoswipe-ui-default',
					  'wp-embed', 'selectWoo', 'stripe', 'wc_stripe_payment_request', 'wc-single-product' );
		foreach( $deregister_scripts as $script_handle ) :
			wp_dequeue_script( $script_handle );
			wp_deregister_script( $script_handle );
		endforeach;
...
//dequeue scripts and styles conditionally on checkout page
add_action( 'wp_enqueue_scripts', 'deregister_scripts_wc_checkout', 99 );
function deregister_scripts_wc_checkout(){
	...
	if( is_page('checkout') ) :
		...
		$deregister_styles = array('decomments1', 'decomments-ie1', 'dashicons', 'upvote-public-style1',
						'toolset-maps-views-filter-distance-frontend-css', 'toolset-select2-css', 'owl-carousel-css',
						'select2-style', 'tmpl-wp-playlist-current-item', 'decomments-css', 'select2' );
		foreach( $deregister_styles as $style_handle ) :
			wp_dequeue_style( $style_handle );
			wp_deregister_style( $style_handle );
		endforeach;
	  ...
	  'zoom', 'flexslider', 'photoswipe', 'wp-embed', 'selectWoo', 'wc-single-product', 'wc-address-i18n1' );
		foreach( $deregister_scripts as $script_handle ) :
			wp_dequeue_script( $script_handle );
			wp_deregister_script( $script_handle );
		endforeach;
