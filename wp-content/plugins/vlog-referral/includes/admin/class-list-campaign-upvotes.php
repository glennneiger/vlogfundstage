...
class Vlog_Campaign_Upvotes extends WP_List_Table {
	...
	private function table_data(){
		...
		if( isset( $_GET['won'] ) && !empty( $_GET['won'] ) ) :
			...
			$prize = vlogref_upvotes_prize_details( $_GET['prize'] );			
		...
		$camp_rankings = vlogref_upvotes_get_campaign_referrals( array('campaign' => $campaign) );
		$camp_prizes = vlogref_upvotes_referral_prizes($campaign);
		$counter = 1;		
		foreach( $camp_rankings as $rank ) :
			$user		= get_userdata($rank['referred_by']);
			$winners	= vlogref_upvotes_get_campaign_winners($rank['campaign']);
			$won_camp  	= vlogref_upvotes_get_user_won_campaigns( $user->ID );
			$winner_str = '';
			...
			if( array_key_exists($user->ID, $winners) ) :
				...
				$cur_prize_details = vlogref_upvotes_prize_details($cur_camp_prize);
			...
			if( !empty( $won_camp ) ) : //Check Other Campaign Won
				foreach( $won_camp as $camp => $prize ) :
					...
					$prize_details = vlogref_upvotes_prize_details($prize);
			...
			$data[] = array( 'position' => $counter,
							...
							'total_signup' => vlogref_upvotes_user_referred_signup_count($rank['campaign'], $user->ID),
