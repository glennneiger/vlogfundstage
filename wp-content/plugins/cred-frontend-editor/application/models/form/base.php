...
abstract class CRED_Form_Base implements ICRED_Form_Base {
    ...
    public function print_form() {
	    ...
	    if ( ! $this->_preview &&
		    ...
		    return $formHelper->error( __( 'User does not have access to this Form', 'wp_cred' ) );
	    }
        ...
        $cred_form_rendering = new CRED_Form_Rendering( $this->_form_id, $my_form_id, $_form_type, $this->_post_id, $actionUri, $this->_preview );
	    ...
	    if (
		    ! $cred_form_rendering->preview && /* 'edit'!=$form_type && (isset($_GET['action']) && $_GET['action'] == 'edit_translation' && 'translation'!=$form_type) && */
            ...
            $cred_form_rendering->is_submit_success = $this->isSubmitted();
        }
        ...
        if ( isset( CRED_StaticClass::$out['fields']['post_fields'] ) && isset( CRED_StaticClass::$out['form_fields_info'] ) ) {
            $formHelper->checkFilesType( CRED_StaticClass::$out['fields']['post_fields'], CRED_StaticClass::$out['form_fields_info'], $cred_form_rendering, $error_files );
        }
	    ...
	    if ( ! $bypass_form
		    ...
		    if ( ! $cred_form_rendering->preview ) {
                ...
                if ( !$bypass_save_form_data ) {
	                ...
	                CRED_Notification_Manager::get_instance()->set_current_attached_data( $form->getForm()->ID, $this->_post_id, $fields['notification']->notifications );
                ...
                if ( is_wp_error( $post_id ) ) {
                    $num_errors++;
                    $cred_form_rendering->add_field_message( $post_id->get_error_message(), 'Post Name' );
                } else {
	    ...
	    if ( $validate
		    ...
		    $output = ( false !== $message ) ? $message : $this->render_form( $messages, $js_messages );
    ...
    public function notify($post_id, $attachedData = null) {
	    ...
	    if (
		    ...
		    CRED_Notification_Manager::get_instance()->add( $post_id, $form->getForm()->ID, $fields['notification']->notifications );
            // send any notifications now if needed
            CRED_Notification_Manager::get_instance()->triggerNotifications( $post_id,
	            array(
		            'event' => 'form_submit',
    ...
    public function isSubmitted() {
        return $this->_cred_form_rendering->isSubmitted();
    }
