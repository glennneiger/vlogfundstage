...
class CRED_PostExpiration {
	...
	function cred_pe_schedule_event_action() {
		if ( $this->_post_expiration_enabled ) {
			...
			$cred_notification_manager = CRED_Notification_Manager::get_instance();
			$cred_notification_manager->removeHooks();
			...
			$cred_notification_manager->addHooks();
			...
			foreach ( $posts_for_notifications as $post_meta ) {
				...
				foreach ( $post_meta->notifications as $key => $notification ) {
					...
					if ( $notification_time <= $now ) {
						...
						CRED_Notification_Manager::get_instance()->sendNotifications( $post_meta->post_id, $form_id, array( $notification ) );
