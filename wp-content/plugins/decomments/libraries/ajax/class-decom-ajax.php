...
//		add_action( 'wp_ajax_deco_block_user', array( $this, 'block_user_comments' ) );
		add_action( 'wp_ajax_decomments_block_unblock_user_comments', array( $this, 'block_unblock_user_comments' ) );

		add_action( 'wp_ajax_delete_all_user_comment', array( $this, 'delete_all_user_comment' ) );

        add_action( 'wp_ajax_nopriv_decomments_login', array( __CLASS__, 'ajax_login' ) );
	}
    ...
    public static function ajax_login() {
        ...
        if ( is_wp_error( $user_signon ) ) {
            wp_send_json( array(
                'loggedin' => false,
                'message' => __( 'Wrong login or password!' )
            ) );
        } else {
            wp_send_json( array(
                'loggedin' => true,
                'message' => __( 'Redirection...' )
            ) );
        }
    }
	...
	public function block_unblock_user_comments() {
		...
		if ( current_user_can( 'moderate_comments' ) ) {
			...
			if ( $action == 'block' ) {
				$res['menu_action']['title']       = __( 'Allow commenting', DECOM_LANG_DOMAIN );
				$res['menu_action']['user_action'] = 'unblock';
				$res['message_in_comment']         = __( 'User blocked', DECOM_LANG_DOMAIN );
			} else {
				$res['menu_action']['title']       = __( 'Disallow commenting', DECOM_LANG_DOMAIN );
				$res['menu_action']['user_action'] = 'block';
				$res['message_in_comment']         = __( 'User unblocked', DECOM_LANG_DOMAIN );
			}
