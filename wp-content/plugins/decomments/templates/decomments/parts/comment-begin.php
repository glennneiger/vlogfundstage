...
if ( in_array( $comment->comment_type, apply_filters( 'decomments_comment_type', array( '' ) ) ) ) {
	...
	$comment_author = apply_filters( 'decomments_get_name_is_email', $comment_author );

	$decom_author_title_bottom_url_arr = apply_filters( 'decom_author_title_bottom_print_site_url', array(
		'show_link'                 => 0,
	...
	$comment->comment_author_url = apply_filters( 'decomments_disable_author_link', $comment->comment_author_url );
...
//	$user_ip            = decomments_get_user_ip();
	...
	$comment_time   = decomments_get_comment_date( '', false, $comment, "$time_format $date_format" );
	$comment_time_t = decomments_get_comment_date( '', false, $comment, $time_format );
	$comment_time_d = decomments_get_comment_date( '', false, $comment, $date_format );
	...
	$expired_time_html = '';
	$expired_time      = decomments_get_expired_time( $comment, $decom_settings );

	if ( decom_is_can_edit_comment( $comment, $expired_time ) ) {
		$expired_time_html = '
                    <span class="decomments-change-list active">
                        <ins> ' . __( 'Time remaining ', DECOM_LANG_DOMAIN ) . ' <em><span class="decomments-expired-edit-time">' . $expired_time . '</span> ' . __( 'min.', DECOM_LANG_DOMAIN ) . '</em> </ins>
	...
	if ( $comment->comment_approved == 'trash' ) {
		...
		$voice = apply_filters( 'decomments_comment_karma_count', $voice, $comment->comment_ID );
		...
		$item_block .= '</span>';
		if ( false === $is_comments_close ) {
			if ( ! isset( $decom_settings['decom_disable_replies'] )
                || (isset( $decom_settings['decom_disable_replies'] )
                    && empty( $decom_settings['decom_disable_replies'])) ) {
		...
		$item_block .= apply_filters( 'decomments_links_bar_on_single_comment', false );
		...
		if ( $attach_ids ) {
			...
			if ( is_array( $attach_ids ) && count( $attach_ids ) > 0 ) {
				...
				foreach ( $attach_ids as $attach_id ) {
					...
					$pictures_block .= wp_get_attachment_image( $attach_id, 'thumbnail', false, array(
