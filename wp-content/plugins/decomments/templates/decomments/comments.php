...
$comments_per_page     = ( isset( $decom_settings['page_comments'] ) && intval( $decom_settings['page_comments'] ) && isset( $decom_settings['comments_per_page'] ) ) ? intval( $decom_settings['comments_per_page'] ) : $number_comments;
$all_comments          = $model_comments->getComments( $post_id );
// Display login form when disable "Comment author must fill out name and email"
// and enable "Users must be registered and logged in to comment"
$decomments_display_login_form = ( isset( $decom_settings['require_name_email'] ) && ! $decom_settings['require_name_email'] && isset( $decom_settings['comment_registration'] ) && $decom_settings['comment_registration'] ) ? true : false;
...
$user_sort             = decomments_get_user_sort( $current_user_id );
...
if ( isset( $ajaxCurrentPage ) && $ajaxCurrentPage ) {
	...
	// fix, when comments page last
	$page                  = $wp_query->query_vars['cpage'];
	$current_comments_page = $page;

	if ( false === strpos( $_SERVER['REQUEST_URI'], 'comment-page-' . $page ) ) {
		$page                  = $pages_count;
		$current_comments_page = $pages_count;
	}

	if ( ! isset( $ajax_page_num ) ) {
		$current_comments_page = 1;
	}
} else {
	$current_comments_page = 1;
...
// This is only paginate comments
if ( isset( $ajax ) ) {
...
<div id="decomments-comment-section" class="decomments-comment-section"
	...
	if ( is_user_logged_in() ) {
		...
		<div class="decomments-enter-row">
			...
			<form id="decomments-enterform" class="decomments-enterform" action="#" method="post" <?php echo ( $decomments_display_login_form ) ? 'style="display:none;"' : ''; ?>>
				<fieldset>
					<div class="de-form-field">
						<label for="decom-name-author"><?php _e( 'Name', DECOM_LANG_DOMAIN ); ?><?php echo ( isset( $decom_settings['require_name_email'] ) && $decom_settings['require_name_email'] == 1 ) ? '<em>*</em>' : ''; ?></label>
						<input id="decom-name-author" type="text" value="<?php echo $_COOKIE['decommentsa']; ?>" />
					</div>
					<div class="de-form-field">
						<label for="decom-mail-author"><?php _e( 'E-mail', DECOM_LANG_DOMAIN ); ?><?php echo ( isset( $decom_settings['require_name_email'] ) && $decom_settings['require_name_email'] == 1 ) ? '<em>*</em>' : ''; ?></label>
			...
			<form id="decomments-loginform" class="decomments-loginform" method="post" <?php echo ( $decomments_display_login_form ) ? 'style="display:block;"' : ''; ?>>
				...
				<?php if ( ! $decomments_display_login_form ) { ?>
	...
	if ( $have_comments ) { ?>
		...
		<div class="decomments-head">
			...
			<form class=" decom_comments_sort decomments-comment-sort" action="#comments" method="post" <?php echo $have_comments ? '' : 'style="display:none;"'; ?>>
				...
				<dl class="de-select de-select-filter">
					...
					if ( $user_sort == 'rate' ) {
						echo '<dt>' . __( 'by Best', DECOM_LANG_DOMAIN ) . '</dt>';
						$class_sort_rate = 'class="current"';
					} else if ( $user_sort == 'newer' || $user_sort == 'newest' ) {
						$class_sort_newer = 'class="current"';
						echo '<dt>' . __( 'by Newest', DECOM_LANG_DOMAIN ) . '</dt>';
					} elseif ( $user_sort == 'older' ) {
						$class_sort_oldest = 'class="current"';
						echo '<dt>' . __( 'by Oldest', DECOM_LANG_DOMAIN ) . '</dt>';
					}
					?>

					<dd name="decom_comments_sort" class="decomments-select">
						<a onclick="decom.sortComments(this)" <?php echo $class_sort_rate; ?> href="#" data-sort="rate"<?php selected( $user_sort, 'rate' ); ?>>
							<i class="decomments-icon-thumb-down"></i> <?php esc_html_e( 'by Best', DECOM_LANG_DOMAIN ); ?>
						</a>
						<a onclick="decom.sortComments(this)" <?php echo $class_sort_newer; ?> href="#" data-sort="newer"<?php selected( $user_sort, 'newer' ); ?>>
							<i class="decomments-icon-thumb-down"></i> <?php esc_html_e( 'by Newest', DECOM_LANG_DOMAIN ); ?>
						</a>
						<a onclick="decom.sortComments(this)" <?php echo $class_sort_oldest; ?> href="#" data-sort="older"<?php selected( $user_sort, 'older' ); ?>>
		...
		<div class="decomments-comment-list<?php echo $da_position; ?>" style="min-height: 250px;">
