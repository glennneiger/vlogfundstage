...
//			print_r( $response );
		...
		// Single update
		$single_update_ver = get_option( 'decomments_single_update' );
		if ( $single_update_ver ) {
			$new_ver = $single_update_ver;
			$response->new_version = $single_update_ver;
		}
		...
		if ( isset( $new_ver ) && isset( $curr_ver ) ) {
			...
			if ( $response !== false && version_compare( $new_ver, $curr_ver, '>' ) ) {
				delete_option( 'decomments_single_update' );
