...
class DECOM_CommentsWalker extends Walker_Comment {
     ...
     */
    public function paged_walk( $elements, $max_depth, $page_num, $per_page, $option = array() ) {
        /* sanity check */
        ...
        foreach ( $elements as $e ) {
            if ( ! $e->$parent_field ) {
                /**if ( deco_comment_get_children_comments( $e->comment_ID ) ) {
                    continue;
                }*/
        ...
        foreach ( $top_level_elements as $e ) {
            ...
            if ( $printed_number >= $per_page  ) {
                break;
            }

            //for the last page, need to unset earlier children in order to keep track of orphans
            if ( $end >= $total_top && $renge_iter < $start ) {
            ...
            if ( $start > 0 ) {
                if ( $renge_iter >= $end && $printed_number >= $per_page ) {
                    break;
                }
            } else {
                if ( $renge_iter > $end && $printed_number >= $per_page ) {
            ...
            if ( $this->display_element( $e, $children_elements, $max_depth, 0, $args, $output ) ) {
                $printed_number++;
            }
        }
