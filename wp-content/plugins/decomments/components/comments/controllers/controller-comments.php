...
class DECOM_Controller_Comments extends DECOM_Controller {
	...
	public function verifyEmail( $get, $post ) {
		...
		if ( empty( $require_name_email ) ) {
			$result = array(
				'require_name_email' => 'disable',
				'result'             => 'success'
			);
		} else if ( isset( $post['email'] ) && strlen( trim( $post['email'] ) ) > 4 && ! is_user_logged_in() ) {
	...
	public function sortComments( $get, $post ) {
        if ( $post['decom_comments_sort'] ) {
            setcookie( 'decomments_sort', $post['decom_comments_sort'], time() + 60 * 60 * 24 * 30, '/' );

			if ( is_user_logged_in() ) {
				update_user_meta( get_current_user_id(), 'decom_comments_sort', $post['decom_comments_sort'] );
			}
		}
