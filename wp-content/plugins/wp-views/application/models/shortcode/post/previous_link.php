 ...
 * @since 2.5.0
 */
class WPV_Shortcode_Post_Previous_Link extends WPV_Shortcode_Base {
	...
	public function get_value( $atts, $content = null ) {
		...
		if ( \OTGS\Toolset\Views\Controller\Compatibility\Wpml::get_instance()->is_wpml_st_loaded() ) {
			...
			$this->user_atts['format'] = wpv_translate(
				'post_control_for_previous_link_format_' . md5( $this->user_atts['format'] ),
				$this->user_atts['format'],
				false,
				$context
			);
			$this->user_atts['link'] = wpv_translate(
				'post_control_for_previous_link_text_' . md5( $this->user_atts['link'] ),
				$this->user_atts['link'],
				false,
				$context
			);
