...
class WP_Offload_SES extends Plugin_Base {
	 ...
	 */
	public function upgrade_routines() {
		$version = get_site_option( 'wposes_version', '0.0.0' );

		if ( version_compare( $version, $GLOBALS['wposes_meta']['wp-offload-ses']['version'], '<' ) ) {
	...
	private function maybe_migrate_from_wpses() {
		$wpses_options = get_site_option( 'wpses_options', false );

		if ( $wpses_options && ! $this->settings->get_settings() ) {
			// Do stuff here.
			foreach ( $wpses_options as $key => $value ) {
				switch ( $key ) {
					case 'from_name':
						$this->settings->set_setting( 'default-email-name', $value );
						break;
					case 'from_email':
						$this->settings->set_setting( 'default-email', $value );
						break;
					case 'reply_to':
						$this->settings->set_setting( 'reply-to', $value );
						break;
					case 'return_path':
						$this->settings->set_setting( 'return-path', $value );
						break;
					case 'access_key':
						$this->settings->set_setting( 'aws-access-key-id', $value );
						break;
					case 'secret_key':
						$this->settings->set_setting( 'aws-secret-access-key', $value );
						break;
					case 'active':
						$this->settings->set_setting( 'send-via-ses', $value );
						break;
					case 'endpoint':
						$value  = explode( '.', $value );
						$region = $value[1];
						$this->settings->set_setting( 'region', $region );
						break;
				}
			}

			$this->settings->set_setting( 'completed-setup', true );
	...
	public function plugin_deactivated_notice() {
		if ( false !== ( $deactivated_notice_id = get_transient( 'wposes_deactivated_notice_id' ) ) ) {
			$title   = __( 'WP Offload SES Activation', 'amazon-s3-and-cloudfront' );
			$message = __( 'WP SES and WP Offload SES cannot both be active. We\'ve automatically deactivated WP SES.', 'amazon-s3-and-cloudfront' );
	...
	public function admin_menu() {
		...
		$submenu['index.php'][] = array( __( 'Offload SES', 'wp-offload-ses' ), 'manage_options', $this->get_plugin_page_url() . '#reports' ); // phpcs:ignore
	...
	public function plugin_load() {
		...
		wp_localize_script(
			...
			array(
				...
				'plugin_url'       => $this->get_plugin_page_url(),
	...
	public function get_verified_senders( $force = false ) {
		$verified_senders = get_site_transient( 'wposes_verified_senders' );

		if ( false === $verified_senders || $force ) {
			$verified_senders = $this->get_aws()->needs_access_keys() ? array() : $this->get_ses_api()->get_identities();
			set_site_transient( 'wposes_verified_senders', $verified_senders, 300 );
		}
	...
	private function check_unverified_senders() {
		...
		$unverified_senders = get_site_transient( 'wposes_unverified_senders' );
