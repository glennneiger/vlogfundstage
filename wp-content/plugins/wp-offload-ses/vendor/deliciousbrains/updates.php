...
namespace DeliciousBrains\WP_Offload_SES\Licences;
	...
	public function enqueue_plugin_update_script() {
		...
		wp_localize_script( $handle,
			...
			array(
				...
				'strings' => array(
					'check_license_again'     => __( 'Check my license again' ),
					'license_check_problem'   => __( 'A problem occurred when trying to check the license, please try again.' ),
					'requires_parent_license' => __( 'Requires a valid license for %s.' )
				),
	...
	function version_update_notice( $basename ) {
		...
		if ( version_compare( $installed_version, $latest_version, '<' ) ) { ?>
			<div style="display: block;" class="updated warning inline-message">
				<strong><?php _ex( 'Update Available', 'A new version of the plugin is available' ); ?></strong> &mdash;
				<?php
